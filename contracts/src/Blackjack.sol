...
contract Blackjack is VRFConsumerBaseV2Plus, ReentrancyGuard {
    ...
    uint8 public constant FLAG_ACTIVE = 0;
    uint8 public constant FLAG_PLAYER_STOOD = 1;
    uint8 public constant FLAG_DEALER_DONE  = 2;
    ...
    constructor(address _vrfCoordinator, bytes32 _keyHash, address _chip, uint256 _subId) payable VRFConsumerBaseV2Plus(_vrfCoordinator) {
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] calldata randomWords) internal override {
        ...
        // Cas spécial: distribution initiale (3 cartes demandées)
        if (randomWords.length == 3 && game.playerCards.length == 0 && game.dealerCards.length == 0) {
            ...
            // Donne 1 carte au croupier
            game.dealerCards.push(drawCard(game, randomWords[2]));
            return;
        }

        // Logique normale pour les tirages ultérieurs
        for (uint256 i = 0; i < randomWords.length; i++) {
            ...
            if (!isFlagSet(game.statusFlags, FLAG_PLAYER_STOOD)) {
                ...
                if (dealerShouldDrawCard(game.dealerCards) ){
    ...
    */
    function isFlagSet(uint8 flags, uint8 bit) public pure returns (bool) {
